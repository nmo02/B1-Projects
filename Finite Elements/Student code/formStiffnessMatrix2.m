function [K, argout] = formStiffnessMatrix2(nodeCoords, IEN, elementType, numGP, CMatrix, BetaMatrix, varargin)
...
%   formStiffnessMatrix().
% BetaMatrix - a #Eq*#Dim-by-#Eq matrix (u-->grad(u)) or a
...
%
% COMMENTS:
% This routine integrates an additional term related to the contribution
% arising directly from the field interpolation. See formStiffnessMatrix()
% for the basic procedure.
%
% Examples:
%   K = formStiffnessMatrix2(nodeCoords, IEN, elementType, numGP, CMat, BMat)
%   K = formStiffnessMatrix2(nodeCoords, IEN, elementType, numGP, CMat, BMat, u)
%   K = formStiffnessMatrix2(nodeCoords, IEN, elementType, numGP, CMat, BMat, [], s)
%   K = formStiffnessMatrix2(nodeCoords, IEN, elementType, numGP, CMat, BMat, u, s)
...
if isa(BetaMatrix,'function_handle')
    B_sample=...
        BetaMatrix(nodeCoords(1,:),u(1,:),zeros(1,numEqU*numDim),s(1,:));
else
    B_sample=BetaMatrix;
end
...
if length(C_sample)~=length(B_sample)
    error('CMatrix and BetaMatrix dimensions are inconsistent');
end
...
%% ==== Local stiffness matrix assembly ==
    function KLocal = formLocalStiffness(elementNodeCoords, shapeFunctions2d,...
            gaussPts, gaussWeights,CMat,BetaMat)
    % forms a stiffness matrix for a given element
        ...
        for l=1:numel(gaussWeights) %loop over Gauss points
            ...
            KLocal=KLocal+gradH'*(CMat*gradH + BetaMat*H)*sqrt(det(J'*J))*gaussWeights(l);
                
        end %loop over Gauss points
    end %formLocalStiffness

    function [KLocal, args_el] = formLocalStiffnessVar(elementNodeCoords, shapeFunctions2d,...
    ...
    % forms a stiffness matrix for a given element
        ...
        for l=1:numel(gaussWeights) %loop over Gauss points
            ...
            KLocal=KLocal+gradH'*(CMatVals*gradH + BetaMatVals*H)*sqrt(det(J'*J))*gaussWeights(l);
        end %loop over Gauss points
    end %formLocalStiffness
...
if ~isnonconstant %linear problem, constant CMatrix
    for m=1:numEl %loop over elements
        ...
        Klocal = formLocalStiffness(elementNodeCoords, shapeFunctions,...
            gaussPts, gaussWeights,CMatrix,BetaMatrix);  %current element's stiffness matrix
