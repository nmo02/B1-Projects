function example7_1%%%% Mechanical equilibrium 3D and 2D - bending cantilever beam
% Plane stress vs 3dQ1 elements

%% ===== Problem Parameters ===== %
domain=[0 0; 10 3]; %rectangular domain
thickness=1;
domain3d=[domain(1,:) 0;  domain(2,:) thickness]; %cuboid domain
% Material properties
youngsModulus = 193e6;
poissonsRatio = 0.253;
...
%% ============ ========== PART 1 (3D) ========== ============
    ...
    function [u2_E,smpGrad,nodeCoords]=go3d(elementSize,smpPts)
        ...
        [u_prescribed_E, Fs_E, prescribedDoF_E, freeDoF_E]=...
            ...
            boundaryTraction3d_E,boundaryDisplacement3d_E,isDirichlet_E,3);
        ...
        %===== Plots ===== %
        drawElements(nodeCoords,boundaryIEN,bndElementType,0,0)
        hold on;
        shift=ones(size(nodeCoords,1),1)*[0 0 1];
        drawElementsInterp(shift+nodeCoords+u2_E*factor,boundaryIEN,bndElementType,s2(1,:),.5);
        drawNodes(shift+nodeCoords+u2_E*factor,boundaryIEN(3));
        end
