...
% Newmark method for a nonlinear problem.
%
% INPUT:
% beta - Newmark beta parameter.
% gamma - Newmark gamma parameter.
% ts - #TimeStepa+1-by-1 matrix of integration time points.
...
% 'iteration', outputFunction - function called at each Newton iteration and
%   supplied with ([i j],ts(i),u,v,a,fDoF,pDoF,Mval,Cval,Kval,Fval,u0,v0,a0,upval,vpval,apval)
%   as input. The output must be one or several matrices. The outputs are
%   collected and returned at the end of the procedure. This option is
%   ignored for linear problems.
...
%     NI is the total number of iterations of 
%
% COMMENTS:
...
% (II) Mass matrix must be non-singular (e.g. a lumped diagonal matrix)
...
for i=2:numTimesteps
    ...
    % evaluate residual at the predited state
    Ct_=Ct(ts(i),u,v);
    Kt_=Kt(ts(i),u,v);
    M_=M(ts(i),u,v);
    Ft_=Ft(ts(i),u,v);
