...
%
% The solution is similar to example4_1, but here we use Voigt strain
% representation and the corresponding form of elasticity tensor.
...
CMatrix_E=elProp.CPlaneStrainEng;
...
%% ============ ========== SOLUTION SUBROUTINE ========== ============
    function [numEl,h,err_s_GP,err_s_C,err_s_N,err_s_SPR,err_u_N,t]=go(elementSize)
        ...
        bndIEN = IENtoBIEN(IEN,boundaryElementIDs,boundaryNodeLocalIDs);
        ...
        K_E = formStiffnessMatrixEng(nodeCoords, IEN, elementType, numGP, CMatrix_E);
        ...
        [u_prescribed_E, Fs_E, prescribedDoF_E, freeDoF_E]=...
            formBC(nodeCoords,bndIEN,elementType1d,numGP1d,...
        ...
        % ===== Stress recovery =====
        [gradu_GP, GPCoords]=recoveryGPEng(u2_E,nodeCoords,IEN,elementType,numGP);
        [gradu_C, CCoords]=recoveryGPEng(u2_E,nodeCoords,IEN,elementType,1);
        [gradu_N]=recoveryAvgEng(u2_E,nodeCoords,IEN,elementType);
        [gradu_SPR]=[1 0 0 0;0 0 0 1;0 1 1 0]*recoveryPatch(u2_E,nodeCoords,IEN,elementType,bndIEN,INE,degrees);
        
        s2_GP=CMatrix_E*gradu_GP;
        s2_C=CMatrix_E*gradu_C;
        s2_N=CMatrix_E*gradu_N;
        s2_SPR=CMatrix_E*gradu_SPR;
        s2_anal_GP=[s11_(GPCoords(:,1),GPCoords(:,2))'; s22_(GPCoords(:,1),GPCoords(:,2))'; s12_(GPCoords(:,1),GPCoords(:,2))'];
        s2_anal_C=[s11_(CCoords(:,1),CCoords(:,2))'; s22_(CCoords(:,1),CCoords(:,2))'; s12_(CCoords(:,1),CCoords(:,2))'];
        s2_anal_N=[s11_(nodeCoords(:,1),nodeCoords(:,2))'; s22_(nodeCoords(:,1),nodeCoords(:,2))'; s12_(nodeCoords(:,1),nodeCoords(:,2))'];
        ...
        if do_plot==1
            factor=1e6;
            figure(3);  hold off;
            drawElementsInterp(nodeCoords+u2_E*factor,IEN,elementType,s2_SPR(2,:)-s2_anal_N(2,:),.5*ones(size(IEN,1),1));
            hold on;
