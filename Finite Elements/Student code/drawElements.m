function drawElements(nodeCoords,IEN,elementType,varargin)
% Draw the mesh using MATLAB patch() function. Values may be specified for
% each element to define the color. See also drawElementsInterp function.
%
% INPUT:
...
% vals (optional) - a scalar or #El-by-1 vector containing scalar values to
%   display as element colors;
% alphas (optional) - a scalar or #El-by-1 vector containing scalar
%   which are interpreted as element opacity;
...
% drawElements(nodeCoords,{IEN,boundaryElementIDs},...) 
% drawElements(nodeCoords,{IEN,{[1;2];[5;6]}},...) 
...
elseif iscell(IEN)&&size(IEN,2)==1 % BIEN
    IEN=cell2mat(IEN);
elseif ~iscell(IEN)
    elementNumbers=1:size(IEN,1); % IEN
else
...
if ~isempty(varargin)&&~isempty(varargin{1})
    vals=varargin{1};
    if isscalar(vals)
        vals=vals*ones(numElements,1);
    elseif numel(vals)~=size(IEN,1)
        error('Number of values does not agree with the number of elements');
    end
else
...
if numel(varargin)>1
    alphas=varargin{2};
    if isscalar(alphas)
        alphas=alphas*ones(numElements,1);
    elseif size(vals)~=size(alphas)
...
if ~ishold %if hold off then erase current axis
    % because hold acts funny on patch objects and we want to imitate
...
if size(perm,1)>1 %strcmpi(elementType(1:2),'3d')
    ...
    color_vals=repmat(color_vals',1,size(perm,1))';
    alphas=repmat(alphas',1,size(perm,1))';
