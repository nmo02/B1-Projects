function [K, argout] = formStiffnessMatrixEng(nodeCoords, IEN, elementType, numGP, CMatrix, varargin)
...
% CMatrix - a 3-by-3 or 6-by-6 matrix (strain-->stress) or a
...
%
% COMMENTS:
% This routine uses the B-matrix. See formStiffnessMatrix for a
% similar procedure using the gradH-matrix.
%
% Examples:
%   K = formStiffnessMatrixEng(nodeCoords, IEN, elementType, numGP, CMat)
%   K = formStiffnessMatrixEng(nodeCoords, IEN, elementType, numGP, CMat, u)
%   K = formStiffnessMatrixEng(nodeCoords, IEN, elementType, numGP, CMat, [], s)
%   K = formStiffnessMatrixEng(nodeCoords, IEN, elementType, numGP, CMat, u, s)
...
numEq=numDim; % Number of scalar equations
numGDoF=numNodes*numEq; %total number of displacement degrees of freedom

if ~ (numEq==2 && length(C_sample)==3) || (numEq==3 && length(C_sample)==6) %check if numEq is integer
...
%% ==== Local stiffness matrix assembly ==
    ...
    % forms a stiffness matrix for a given element
        ...
        for l=1:numel(gaussWeights) %loop over Gauss points
            [~, naturalDerivatives]=shapeFunctions(gaussPts(l,1),gaussPts(l,2));
            ...
            if (numDim==2)&&(numEq==2)
                B=zeros(3,numLocalDoF);
                B(1,1:numEq:end)=spatialDerivatives(:,1)';
                B(2,2:numEq:end)=spatialDerivatives(:,2)';
                B(3,1:numEq:end)=spatialDerivatives(:,2)';
                B(3,2:numEq:end)=spatialDerivatives(:,1)';
            elseif (numDim==3)&&(numEq==3)
                B=zeros(6,numLocalDoF);
                B(1,1:numEq:end)=spatialDerivatives(:,1)';
                B(2,2:numEq:end)=spatialDerivatives(:,2)';
                B(3,3:numEq:end)=spatialDerivatives(:,3)';
                B(4,2:numEq:end)=spatialDerivatives(:,3)';
                B(4,3:numEq:end)=spatialDerivatives(:,2)';
                B(5,3:numEq:end)=spatialDerivatives(:,1)';
                B(5,1:numEq:end)=spatialDerivatives(:,3)';
                B(6,1:numEq:end)=spatialDerivatives(:,2)';
                B(6,2:numEq:end)=spatialDerivatives(:,1)';
            else
                error('2D or 3D elasticity problem expected');
            end
            
            KLocal=KLocal+B'*CMat*B*sqrt(det(J'*J))*gaussWeights(l);
    ...
    % forms a stiffness matrix for a given element
        ...
        for l=1:numel(gaussWeights) %loop over Gauss points
           ...
           if (numDim==2)&&(numEq==2)
            ...
            KLocal=KLocal+B'*CMatVals*B*sqrt(det(J'*J))*gaussWeights(l);
        end %loop over Gauss points
    end %formLocalStiffness
